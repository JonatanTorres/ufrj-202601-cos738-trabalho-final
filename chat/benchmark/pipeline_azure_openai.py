"""Benchmark do pipeline completo para os modelos Azure OpenAI.

Igual ao `pipeline_anthropic_haiku.py`, mas força o pipeline a rodar com os
modelos Azure OpenAI (GPT-5.4 e GPT-5.4 mini). Diferente dos scripts da
Anthropic — onde Sonnet e Haiku rodam separados para controlar custo —, aqui
os dois modelos rodam no mesmo script.
...
Uso:
    cd chat
    python benchmark/pipeline_azure_openai.py
"""
import csv
import json
import time
...
def main() -> None:
    ...
    output = Path(__file__).parent / "results_pl_azure_openai.csv"
