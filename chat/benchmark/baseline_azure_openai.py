"""Benchmark de verdadeiro/falso para modelos Azure OpenAI.

Executa cada pergunta em cada modelo e salva as respostas em
results_bl_azure_openai.csv.

Pré-requisitos:
    - AZURE_OPENAI_API_KEY e AZURE_OPENAI_ENDPOINT definidas no `chat/.env`.
    - Os deployments `gpt-5.4` e `gpt-5.4-mini` criados no recurso Azure.

Uso:
    cd chat
    python benchmark/baseline_azure_openai.py
"""
import csv
import os
import time
...
# chave -> nome do deployment no Azure
MODELS = {
...
def ask(client: AzureOpenAI, deployment: str, question: str) -> str:
...
def main() -> None:
    ...
    if not api_key or not endpoint:
        raise RuntimeError(
            "AZURE_OPENAI_API_KEY ou AZURE_OPENAI_ENDPOINT não encontradas no .env"
        )
    ...
    for model_key, deployment in MODELS.items():
        print(f"\n=== {model_key} ({deployment}) ===")
        for i, question in enumerate(QUESTIONS, 1):
            t0 = time.time()
            answer = ask(client, deployment, question)
    ...
    output = Path(__file__).parent / "results_bl_azure_openai.csv"
